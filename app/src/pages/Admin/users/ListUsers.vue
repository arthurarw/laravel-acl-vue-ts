...
<template>
  ...
  <form action="#" method="get" @submit.prevent="loadUsers(1)" class="flex mt-4">
    <input type="text" name="filter" placeholder="Filtrar" v-model="filter" class="shadow appearance-none border rounded w-full py-2 px-3 mr-4 text-grey-darker">
    <select name="totalPerPage" v-model="totalPerPage" class="shadow appearance-none border rounded py-2 px-3 mr-4 text-grey-darker">
    ...
    <button type="submit" class="flex-no-shrink p-2 border-2 rounded text-teal border-teal hover:text-white hover:bg-teal">Filtrar</button>
  </form>
      ...
      <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
        <tr v-for="user in userStore.users" :key="user.id">
          ...
          <td class="px-4 py-4 text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">
            <router-link :to="{ name: 'users.show', params: { id: user.id } }" class="flex-no-shrink p-2 mr-2 border-2 rounded hover:text-white text-green border-green hover:bg-slate-500">Detalhes</router-link>
            <router-link :to="{ name: 'users.edit', params: { id: user.id } }" class="flex-no-shrink p-2 ml-1 mr-2 border-2 rounded hover:text-white text-green border-green hover:bg-slate-500">Editar</router-link>
          </td>
        </tr>
      </tbody>
    </table>
