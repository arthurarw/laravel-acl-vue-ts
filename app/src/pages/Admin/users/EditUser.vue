...
<template>
  ...
  <form action="#" method="post" @submit.prevent="updateUser">
    <div class="mt-4">
      ...
      <input type="text" name="name" placeholder="Nome" v-model="form.name" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 p-2" :class="{ 'has-error': errors.validations.hasOwnProperty('name') }">
    </div>
    <div class="mt-4">
      <label for="email" class="block text-sm font-medium leading-6 text-gray-900">E-mail:</label>
      <input type="email" name="email" placeholder="E-mail" v-model="form.email" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 p-2" :class="{ 'has-error': errors.validations.hasOwnProperty('email') }">
    </div>
    <div class="mt-4">
      <label for="senha" class="block text-sm font-medium leading-6 text-gray-900">Senha:</label>
      <input type="password" name="password" placeholder="Senha" v-model="form.password" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 p-2" :class="{ 'has-error': errors.validations.hasOwnProperty('password') }">
    </div>
    <button type="submit" :disabled="loading" class="mt-4 w-full bg-slate-500 hover:bg-slate-400 text-white font-bold py-2 px-4 border-b-4 border-slate-700 hover:border-slate-500 rounded">
