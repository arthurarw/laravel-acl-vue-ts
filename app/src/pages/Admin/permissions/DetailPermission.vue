    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
      ...
      <li aria-current="page">
        <div class="flex items-center">
          ...
          <span class="text-sm font-medium text-gray-500 ms-1 md:ms-2 dark:text-gray-400">Permissãos</span>
        </div>
      </li>
    </ol>
  </nav>

  <h2 class="mt-4 text-4xl font-extrabold">Detalhes da Permissão</h2>

  <div v-if="loading">Carregando...</div>
  <ul class="mt-4">
    <li><strong>Name: </strong>{{ permission?.description }}</li>
    <li><strong>Route Name: </strong>{{ permission?.name }}</li>
  </ul>

  <div v-if="permission" class="mt-4" :class="{ 'disabled': loadingDelete }">
    <span v-if="loadingDelete">Deletando...</span>
    <a v-else href="#" @click.prevent="deletePermission" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Deletar o Permissão {{ permission.description }}</a>
  </div>
</template>
