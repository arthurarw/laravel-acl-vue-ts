...
export default {
  ...
  setup() {
    ...
    const updatePermission = () => {
      reset()
      loading.value = true
      permissionStore.update(permissionId.value, { ...form })
...
<template>
  ...
  <h2 class="mt-4 text-4xl font-extrabold">Editar Permissão</h2>
  ...
  <form action="#" method="post" @submit.prevent="updatePermission">
    <div class="mt-4">
      ...
      <input type="text" name="name" placeholder="Nome" v-model="form.description" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 p-2" :class="{ 'has-error': errors.validations.hasOwnProperty('name') }">
    </div>
    <div class="mt-4">
      <label for="description" class="block text-sm font-medium leading-6 text-gray-900">Nome da Rota:</label>
      <input type="text" name="description" placeholder="Ex: user.index" v-model="form.name" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 p-2" :class="{ 'has-error': errors.validations.hasOwnProperty('route_name') }">
    </div>
