...
<template>
  <h2 class="mt-4 text-4xl font-extrabold">Permissões</h2>
  <div class="flex">
    <router-link :to="{ name: 'permissions.create' }" class="py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Criar
    ...
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
      <thead class="bg-gray-50 dark:bg-gray-800">
        <tr>
          ...
          <th class="py-3.5 px-4 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Route Name</th>
      ...
      <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
        <tr v-for="permission in permissionStore.permissions" :key="permission.id">
          ...
          <td class="px-4 py-4 text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">
            <router-link :to="{ name: 'permissions.show', params: { id: permission.id } }" class="p-2 mr-2 border-2 rounded flex-no-shrink hover:text-white text-green border-green hover:bg-slate-500">Detalhes</router-link>
            <router-link :to="{ name: 'permissions.edit', params: { id: permission.id } }" class="p-2 ml-1 mr-2 border-2 rounded flex-no-shrink hover:text-white text-green border-green hover:bg-slate-500">Editar</router-link>
          </td>
        </tr>
      </tbody>
    </table>
